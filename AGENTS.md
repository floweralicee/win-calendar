...
- **State**: local component state only, no global store. `App` owns the `config` gate, the fetched `winsByDate`, the visible month, the selected win (for detail modal), whether the journal composer is open, and the `activeView` (`'month' | 'bloom' | 'year' | 'list' | 'goals'`). `activeView` is passed to `Calendar`, which renders the appropriate view. `GoalsView` manages its own goals state internally.
...
| `src/Calendar.tsx` | ~250 | Month grid + win titles + 5-button view toggle (Month / Bloom / Year / List / Goals). Renders the appropriate sub-view based on `activeView`. |
...
| `server/src/prompts.ts` | ~40 | The system prompt for win extraction. Asks for `areas[]` (1–3 life areas). Strict "JSON only, wins must be real" instruction. |
| `server/src/goals-store.ts` | ~85 | Reads/writes `~/.win-calendar/goals.json` (mode 0600). Exports `Goal` type, `readGoals()`, `addGoal()`, `updateGoal()`, `deleteGoal()`. |
| `server/src/eisenhower.ts` | ~75 | Computes the Eisenhower grid from active goals + win history. Urgency = deadline ≤ 6 weeks AND < 2 wins/week in that area over past 4 weeks. Returns `urgentImportant[]` and `notUrgentImportant[]`. |
| `server/src/routes/goals.ts` | ~90 | `GET/POST /api/goals`, `PATCH/DELETE /api/goals/:goalId`. Full CRUD for goals. |
