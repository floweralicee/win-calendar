...
- **Phase 7 — Momentum Heatmap** *(done, branch `feature/aliceos`)*: a third view (Year). GitHub-style 52×7 day grid; cell fill color = dominant life area for that day; fill opacity = win count intensity (4 stops). Today has an inset outline ring. Hover any filled cell for a tooltip showing the day's wins. Three-button toggle in the header: Month | Bloom | Year.
...
- **State**: local component state only, no global store. `App` owns the `config` gate, the fetched `winsByDate`, the visible month, the selected win (for detail modal), whether the journal composer is open, and the `activeView` (`'month' | 'bloom' | 'year'`). `activeView` is passed to `Calendar`, which renders the month grid, `<BloomView>`, or `<HeatmapView>` accordingly.
...
| `src/Calendar.tsx` | ~230 | Month grid + win titles inside day cells + view toggle (Month / Bloom / Year). Owns `buildMonthGrid()`, weekday/month labels, the ‹/› nav chevrons, the clickable month label, the three-button view toggle, and the **Journal** button. Renders `<BloomView>` or `<HeatmapView>` when `activeView` is non-month, or the grid when `activeView === 'month'`. |
...
| `src/HeatmapView.tsx` | ~210 | Year heatmap. Builds a 52-week × 7-day grid from today going back 52 weeks. Each cell's background is the dominant area's RGBA color at an intensity scaled to win count (4 opacity stops). Today gets an inset outline. Future cells are transparent placeholders. Hover any filled cell for a tooltip with the day's wins. |
| `src/vite-env.d.ts` | ~6 | Vite client types + the `*.md?raw` module declaration (still used by the example fixture). |
| `src/styles.css` | ~900 | Design tokens (including 5 `--ring-*` area color tokens) + calendar, area dots, win-title, detail modal, onboarding, journal composer, app-status, view toggle, bloom, and year heatmap styles (cells, month labels, day labels, tooltip, legend). Single stylesheet. |
...
| `desktop/src/renderer/styles.css` | ~200 | Transparent background, speech bubble, textarea, action buttons, pet sprite styles. |
