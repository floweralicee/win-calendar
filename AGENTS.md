...
- **Phase 5 — Focus Mode** *(in progress, branch `feature/desktop`)*: the pet gains a Focus Challenge flow. Double-click → "What's on your mind today?" → Claude rewrites the output-based task as a simple input-based one (e.g. "Work on thesis for 25 minutes") → user sees the reframed task and picks **Accept** or **Nah, something else** (loops back to input) → old-school blue monospace countdown timer → on completion, "What did you accomplish in X mins?" → user types what they did → burp fires. See `PLAN.md` for the full spec.
...
| `server/src/routes/reframe.ts` | ~30 | `POST /api/reframe`: takes `{ task: string }`, calls Claude with the reframe prompt, returns `{ reframed: string, durationMins: number }`. |
...
| `server/src/prompts.ts` | ~30 | Two prompts: win extraction ("JSON only, wins must be real") and task reframe (output → input, plain English, returns `{ reframed, durationMins }`). |
...
| File | Lines | Purpose |
|------|-------|---------|
| `desktop/package.json` | ~40 | Electron + React + electron-builder deps. Scripts: `start` (prod), `dev` (watch), `build:main`, `build:renderer`. |
...
| `desktop/src/main.ts` | ~140 | Electron main process. Frameless transparent always-on-top window; `Cmd+Shift+H` toggle; tray menu; single-instance lock; loads `dist-renderer/` when present (else `HANA_VITE=1` + `localhost:5174`); IPC: `journal:submit`, `pet:moveBy`, `pet:setBubbleVisible`, `task:reframe`. |
| `desktop/src/preload.ts` | ~40 | `contextBridge` — `window.hana.submitJournal()`, `moveBy()`, `setBubbleVisible()`, `reframeTask(raw: string): Promise<{ reframed: string, durationMins: number }>`. |
| `desktop/src/renderer/Pet.tsx` | ~320 | React component. Four base states (`active`/`eat`/`sleep`/`touch`) + Focus Mode flow states (`reframing`/`confirm`/`timer`/`debrief`). Sleep timer (30 s), drag via `document` mouse events. Focus flow: open prompt → reframe via Claude → Accept/Nah confirm → countdown timer → debrief input → burp. |
| `desktop/src/renderer/index.html` | ~10 | Electron renderer HTML entry. |
| `desktop/src/renderer/index.tsx` | ~10 | React `createRoot` mount. |
| `desktop/src/renderer/styles.css` | ~280 | Transparent background, speech bubble, textarea, action buttons, pet sprite styles. Focus Mode additions: challenge card, Accept/Nah button pair, blue monospace countdown timer. |
| `desktop/src/renderer/hana.d.ts` | ~15 | TypeScript ambient declaration for `window.hana`. |
| `hana-icon/active.svg` | 4 | Default/idle pet sprite. |
| `hana-icon/eat.svg` | 4 | Pet sprite when textbox is open (user journaling). |
| `hana-icon/sleep.svg` | 4 | Pet sprite after 30 s of inactivity. |
| `hana-icon/touch.svg` | 4 | Pet sprite while user drags the window. |
